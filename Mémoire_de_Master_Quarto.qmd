...
For Arabica, ARIMAX and XGBoost achieve near-identical R² values of 0.821 and 0.818 respectively, confirming that the autoregressive structure of price dynamics carries predictive information that neither purely cross-sectional nor purely tree-based specifications can fully replicate. The forecast trajectory plots show that both models track the broad price level and direction correctly throughout the 2021–2024 test period, including the gradual decline from the 2022 peak, though both exhibit the typical one-step-ahead lag at turning points. Random Forest follows closely at R² = 0.807, while MLR achieves a solid R² = 0.784 — a result that reflects the strong trending co-movement between Arabica prices and the macroeconomic variables in levels, rather than genuine causal predictive power. The ANN and SVM trail at 0.453 and 0.696 respectively, for reasons discussed in the limitations section.

For Robusta, the performance hierarchy shifts markedly. ARIMAX delivers the strongest result of the entire study with R² = 0.929, driven by the strongly persistent autoregressive structure of Robusta prices over the 2021–2024 test period — a period characterised by a near-monotonic upward trend that an AR model with high persistence coefficient naturally tracks well. XGBoost follows at R² = 0.796, while Random Forest and ANN achieve 0.692 and 0.668 respectively. The most striking result is the collapse of MLR to R² = 0.276, with forecast trajectories that oscillate erratically around the observed series and produce large directional errors. This failure reflects the well-documented sensitivity of OLS to multicollinearity in short samples: with only 153 training observations and 14 highly collinear predictors — as documented in the PCA analysis where Production, Consumption and Exports load almost identically on the first principal component — the coefficient estimates are unstable and the model cannot generalise to the test period. This result underscores the importance of temporal dynamics in commodity price modelling and confirms that a static linear specification is insufficient for Robusta price prediction over the available sample.
...
The disaggregated robustness check provides strong ex-post validation of the production-weighted index methodology adopted in this study. Replacing the two aggregated climate indices with the 28 individual national series systematically deteriorates out-of-sample R² across all four ML models and both coffee varieties, with every ΔR² value negative. The degradation is moderate for Random Forest (ΔR² of −0.038 for Arabica, −0.112 for Robusta) and XGBoost (−0.020 and −0.058), but catastrophic for SVM and ANN, which produce negative R² values — worse than a naive mean forecast — under the disaggregated specification.
...
All six models estimated in this study assume a static relationship between coffee prices and their predictors — that is, the coefficients or feature importances are fixed over time (or re-estimated on an expanding window that treats all historical observations equally). This assumption is at odds with the documented structural changes in the coffee market over the analysis period, including the progressive financialisation of commodity markets from the mid-2000s onward, the post-2008 shift in monetary policy regimes, and the supply chain disruptions of the COVID-19 period. A model estimated on pre-2010 data may assign different weights to financial variables than one estimated on post-2015 data, and a static specification cannot capture this evolution. Time-varying parameter models — such as state space models with Kalman filter updating, or rolling-window ML specifications with a fixed rather than expanding training window — would allow the predictor relationships to evolve over time and may improve performance in structurally shifting environments. This extension is left for future research.

# Conclusion

This study set out to assess the predictive accuracy of six forecasting models — two econometric benchmarks (ARIMAX and Multiple Linear Regression) and four machine learning algorithms (Random Forest, XGBoost, Support Vector Machine and Artificial Neural Network) — applied to monthly Arabica and Robusta coffee futures prices over the 2000–2023 period. Using a walk-forward expanding window validation protocol with an 80/20 train/test split, models were evaluated on three out-of-sample metrics — R², MSE and MAE — across a rich predictor set spanning macroeconomic, fundamental, climatic and speculative variables.

The principal empirical finding is that ARIMAX and XGBoost emerge as the strongest models for both coffee varieties, achieving out-of-sample R² values of 0.821 and 0.818 for Arabica and 0.929 and 0.796 for Robusta respectively. The strong performance of ARIMAX confirms that the autoregressive structure of commodity price dynamics — the persistence of prices around their own lagged values — is a primary source of short-run predictability, and that exogenous regressors add meaningful information on top of this autoregressive baseline. The competitive performance of XGBoost demonstrates that gradient boosting algorithms can identify non-linear interactions among predictors that the linear ARIMAX structure cannot accommodate, and that their built-in regularisation provides robustness against overfitting on the relatively small training samples available for this study. The Multiple Linear Regression benchmark performs well for Arabica but collapses for Robusta (R² = 0.276), confirming that a static linear specification without temporal dynamics is insufficient when the estimation sample is short and predictors are highly collinear.

The variable importance analysis reveals that financial and speculative variables dominate the predictor rankings for both varieties. For Arabica, the COT net Managed Money position and Sugar No.11 prices emerge as the two most influential predictors, reflecting the financialisation of the Coffee C futures market and the land substitution dynamics between coffee and sugar in Brazil. For Robusta, WTI crude oil and CPI rank first, consistent with the energy-intensive production logistics of the Vietnamese and Indonesian supply chains and the price-sensitive demand base for soluble coffee. Climate variables — despite their established agronomic relevance — contribute relatively little predictive power at monthly frequency, a finding attributed to the lagged transmission of climate shocks through supply channels that operates over horizons of six to twelve months and cannot be captured by a contemporaneous monthly model.

The disaggregated robustness check provides strong ex-post validation of the production-weighted climate index methodology. Replacing the two aggregated indices with the 28 individual national series systematically deteriorates out-of-sample performance across all four ML models, with SVM and ANN producing negative R² values under the disaggregated specification. This result confirms that the fixed-weight aggregation does not lose predictive information relative to the raw national series — on the contrary, it improves the climate signal by averaging out country-specific noise while preserving the weighted contribution of the most important producers.

Several limitations constrain the interpretation of these results. The estimation of ML models on non-stationary levels raises spurious regression concerns that cannot be fully resolved without cointegration- based specifications. The ANN exhibits instability under walk-forward re-estimation due to random weight initialisation, and the SVM is estimated with fixed hyperparameters that are likely suboptimal. The national climate averages are computed as simple means across ADM1 regions without intra-country production weights, attenuating the climate signal in countries with large non-producing territories. Finally, all specifications assume static predictor relationships over a 24-year period that encompasses major structural breaks in the coffee market, including the financialisation of the 2000s and the post-pandemic supply chain disruptions.

These limitations point to several avenues for future research. Cointegration-based error correction models — such as a Vector Error Correction Model (VECM) augmented with ML residual correction — would address the non-stationarity concern while preserving the long-run equilibrium information contained in the levels. The incorporation of lagged climate variables at horizons of three, six and twelve months would better align the climate signal with the agronomic production cycle and likely improve the predictive contribution of temperature and precipitation. Time-varying parameter specifications, such as state space models with Kalman filter updating or rolling-window ML models, would allow predictor relationships to evolve with the structural changes documented in the coffee market over the analysis period. Finally, the use of remote sensing data on coffee cultivation area at ADM1 resolution — such as those produced by the Global Coffee Watch initiative — would enable intra-country production weighting and produce more targeted climate indices for the major producing countries.

From a practical standpoint, the results of this study suggest that market participants, commodity traders and agribusiness analysts seeking short-run price signals for Arabica and Robusta coffee would benefit most from monitoring speculative positioning in the futures market — as captured by the CFTC COT Disaggregated report — alongside energy prices and the broader macroeconomic environment, rather than focusing primarily on supply-side fundamentals or climate conditions at monthly frequency. The walk-forward validation protocol used throughout this study ensures that these conclusions reflect genuine out-of-sample predictive performance rather than in-sample fit, and the consistent results across multiple model families strengthen their reliability.
