...
fit_mlr <- function(df, var_dep, n_train) {
  ...
  # STEP 2 — Drop any row with at least one missing value.
  # This is what restricts the Arabica sample to start in
  # June 2006 (the first month with COT data available).
  lignes_completes <- complete.cases(df_modele)
  df_modele <- df_modele[lignes_completes, ]

  # STEP 3 — Split into the initial training set and the test
...
fit_rf <- function(df, var_dep, n_train) {
  ...
  # safety check as in the MLR function.
  vars_exo_disponibles <- intersect(vars_exo, colnames(df))
  vars_a_garder <- c(var_dep, vars_exo_disponibles)
  df_modele <- df[, vars_a_garder]
  ...
  # STEP 3 — Min-Max scale every column (including the
  ...
  # STEP 4 — Split into the initial training set and the test set.
  ...
  # STEP 6 — Variable importance scores from the forest grown
...
fit_xgb <- function(df, var_dep, n_train) {
  ...
  # STEP 1 — Keep only the columns this model needs.
  vars_exo_disponibles <- intersect(vars_exo, colnames(df))
  vars_a_garder <- c(var_dep, vars_exo_disponibles)
  df_modele <- df[, vars_a_garder]
  ...
  # STEP 4 — Separate the predictor matrix from the target
