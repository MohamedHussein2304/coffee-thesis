...
fit_mlr <- function(df, var_dep, n_train) {
  ...
  # vars_exo is defined once in the "Results" setup chunk and
...
fit_arimax <- function(df, var_dep, n_train) {
  ...
  #     feasible across the ~40 re-estimations of the loop
  ...
  # model rather than to any of the ~40 models re-estimated
...
This chunk estimates the Random Forest model for both series. The implementation follows the same walk-forward structure as MLR and ARIMAX above, with two differences specific to this model family: all variables, including the dependent variable, are Min-Max scaled to [0, 1] before estimation (see Section 3.5.3), and the forest is grown with 500 trees at each step with importance recording enabled. The variable importance scores from the final iteration of the loop are retained and used in the importance plot chunk later in the document.
...
fit_xgb <- function(df, var_dep, n_train) {
  ...
  list(actual = valeurs_observees, predicted = predictions,
       model  = modele_xgb)   # model from the last (largest) window
}
...
tuning_svm_ara <- tuner_svm(df_arabica_levels, "prix_arabica", n_train_ara_lev)
tuning_svm_rob <- tuner_svm(df_robusta_levels, "prix_robusta", n_train_rob_lev)
...
#   n_train : size of the initial training window (number of
#             rows), computed earlier in the preprocessing chunk
#   cost, gamma : hyperparameters selected by the light tuning
#             search above, passed in rather than hard-coded
#
# What the function returns:
#   actual    : the observed (scaled) prices in the test period
#   predicted : the walk-forward one-step-ahead forecasts
...
#
# Hyperparameters: cost (regularisation) and gamma (kernel
# bandwidth) are passed in as arguments rather than hard-coded,
# so that the values selected by the tuning search above are
# used directly. cost controls the trade-off between margin
...
fit_svm <- function(df, var_dep, n_train, cost = 1, gamma = 0.1) {
  ...
  # fixed at the values selected by the pre-loop tuning search
...
# search above, separately for each series.
# ----------------------------------------------------------
...
The SVM with a radial basis function kernel and tuned hyperparameters delivers a markedly improved out of sample performance relative to earlier untuned specifications. The light grid search selects $cost = 5$ and $gamma = 0.05$ for both Arabica and Robusta, the combination minimising validation MSE within the chronological sub split of each initial training window. With these tuned hyperparameters, the SVM achieves an out of sample $R^2$ of 0.813 for Arabica and 0.723 for Robusta, a substantial improvement over the untuned fixed values of $cost = 1$ and $gamma = 0.1$ used in earlier specifications, and now comparable to or exceeding several of the other ML models for both series. This improvement confirms that the SVM's earlier underperformance was indeed driven in large part by the arbitrary choice of hyperparameters rather than by a fundamental limitation of the kernel based approach for this forecasting problem. The single pre loop tuning search remains a simplification relative to a fully nested walk forward procedure, which would re optimise the hyperparameters at every step but was computationally prohibitive given the sample size and the six model comparative framework of this study.
...
tuning_ann_ara <- tuner_ann(df_arabica_levels, "prix_arabica", n_train_ara_lev)
tuning_ann_rob <- tuner_ann(df_robusta_levels, "prix_robusta", n_train_rob_lev)
...
#                   14x5 + 5x1 = 75 connection weights to
#                   estimate, a manageable number relative to
#                   the ~170-200 training observations available
...
# (MLR, ARIMAX, RF, XGBoost, SVM, ANN), placing the two
...
The four model functions below (Random Forest, XGBoost, SVM, ANN) are exactly the same functions used in Section 4.3–4.6, with a single difference: the variable list passed to them is `vars_exo_dis` (40 disaggregated predictors) instead of `vars_exo` (14 baseline predictors). Rather than repeating four long functions with only one line changed, this chunk writes each function once and calls it with the disaggregated variable list and the disaggregated dataset.
...
df_robusta_dis <- preparer_niveaux_dis(
  dataset_disagg[!is.na(dataset_disagg$prix_robusta), ], "prix_robusta")
...
# Identical in structure to fit_rf(), fit_xgb(), fit_svm() and
# fit_ann() defined earlier in the document. The only
# difference is an added vars_exo_vec argument, since these
# functions are reused below with both the disaggregated list
# (vars_exo_dis, this chunk) and the variety-specific lists
# (Section 4.9) without copying the function body again.
# ----------------------------------------------------------
...
# Each model function is called once for Arabica and once for
# Robusta, using the disaggregated dataset and the 40-variable
# disaggregated predictor list defined in Step 2.
...
res_rf_dis_ara  <- fit_rf_generique(df_arabica_dis, "prix_arabica",
                                    vars_exo_dis, n_train_ara_dis)
cat("Running disaggregated RF — Robusta...\n")
res_rf_dis_rob  <- fit_rf_generique(df_robusta_dis, "prix_robusta",
                                    vars_exo_dis, n_train_rob_dis)

cat("Running disaggregated XGBoost — Arabica...\n")
res_xgb_dis_ara <- fit_xgb_generique(df_arabica_dis, "prix_arabica",
                                     vars_exo_dis, n_train_ara_dis)
cat("Running disaggregated XGBoost — Robusta...\n")
res_xgb_dis_rob <- fit_xgb_generique(df_robusta_dis, "prix_robusta",
                                     vars_exo_dis, n_train_rob_dis)

cat("Running disaggregated SVM — Arabica...\n")
res_svm_dis_ara <- fit_svm_generique(df_arabica_dis, "prix_arabica",
                                     vars_exo_dis, n_train_ara_dis)
cat("Running disaggregated SVM — Robusta...\n")
res_svm_dis_rob <- fit_svm_generique(df_robusta_dis, "prix_robusta",
                                     vars_exo_dis, n_train_rob_dis)

cat("Running disaggregated ANN — Arabica...\n")
res_ann_dis_ara <- fit_ann_generique(df_arabica_dis, "prix_arabica",
                                     vars_exo_dis, n_train_ara_dis)
cat("Running disaggregated ANN — Robusta...\n")
res_ann_dis_rob <- fit_ann_generique(df_robusta_dis, "prix_robusta",
                                     vars_exo_dis, n_train_rob_dis)


# ----------------------------------------------------------
# STEP 6 — COMPILE RESULTS INTO A COMPARISON TABLE
# For each model and series, the baseline R² (already computed
# in metriques_all from Section 4.7) is placed next to the
# disaggregated R² computed just above, and the difference
# between the two (Delta_R2) is reported. Built here, displayed
# in the next chunk.
# ----------------------------------------------------------
...
df_robusta_vs <- preparer_niveaux_vs(
  dataset_robusta_vs[!is.na(dataset_robusta_vs$prix_robusta), ],
...
All six models are re-estimated under this specification using the same walk-forward protocol as the baseline. The Random Forest, XGBoost, SVM and ANN models reuse the generic functions (`fit_rf_generique`, `fit_xgb_generique`, `fit_svm_generique`, `fit_ann_generique`) already defined in the disaggregated robustness check above, called here with the variety-specific variable lists instead of the disaggregated one. ARIMAX and MLR did not yet have a reusable generic version, so the two functions below generalise `fit_arimax()` and `fit_mlr()` from Section 4.1–4.2 by adding a `vars_exo_vec` argument, following the same pattern.

```{r modeles_vs, message=FALSE, warning=FALSE}

# ----------------------------------------------------------
# GENERIC ARIMAX AND MLR FUNCTIONS
# Identical in every respect to fit_arimax() and fit_mlr()
# defined in Sections 4.1 and 4.2, except that the predictor
# list is passed in as an argument (vars_exo_vec) instead of
# always using the global vars_exo. This allows the exact same
# function to be reused for the baseline, and here for the
...
# ----------------------------------------------------------
# RUN ALL SIX MODELS ON BOTH SERIES, VARIETY-SPECIFIC SPECIFICATION
# ----------------------------------------------------------

res_arimax_ara_vs <- fit_arimax_generique(df_arabica_vs, "prix_arabica",
                                          vars_exo_ara_vs, n_train_ara_vs)
res_arimax_rob_vs <- fit_arimax_generique(df_robusta_vs, "prix_robusta",
                                          vars_exo_rob_vs, n_train_rob_vs)

res_mlr_ara_vs <- fit_mlr_generique(df_arabica_vs, "prix_arabica",
                                    vars_exo_ara_vs, n_train_ara_vs)
res_mlr_rob_vs <- fit_mlr_generique(df_robusta_vs, "prix_robusta",
                                    vars_exo_rob_vs, n_train_rob_vs)

res_rf_ara_vs <- fit_rf_generique(df_arabica_vs, "prix_arabica",
                                  vars_exo_ara_vs, n_train_ara_vs)
res_rf_rob_vs <- fit_rf_generique(df_robusta_vs, "prix_robusta",
                                  vars_exo_rob_vs, n_train_rob_vs)

res_xgb_ara_vs <- fit_xgb_generique(df_arabica_vs, "prix_arabica",
                                    vars_exo_ara_vs, n_train_ara_vs)
res_xgb_rob_vs <- fit_xgb_generique(df_robusta_vs, "prix_robusta",
                                    vars_exo_rob_vs, n_train_rob_vs)

res_svm_ara_vs <- fit_svm_generique(df_arabica_vs, "prix_arabica",
                                    vars_exo_ara_vs, n_train_ara_vs)
res_svm_rob_vs <- fit_svm_generique(df_robusta_vs, "prix_robusta",
                                    vars_exo_rob_vs, n_train_rob_vs)

res_ann_ara_vs <- fit_ann_generique(df_arabica_vs, "prix_arabica",
                                    vars_exo_ara_vs, n_train_ara_vs)
res_ann_rob_vs <- fit_ann_generique(df_robusta_vs, "prix_robusta",
                                    vars_exo_rob_vs, n_train_rob_vs)


# ----------------------------------------------------------
# PERFORMANCE METRICS — STACK ALL SIX MODELS, BOTH SERIES
# ----------------------------------------------------------
...
# For each model and series, the baseline R² (from
# metriques_all, Section 4.7) is placed next to the
# variety-specific R² computed just above.
# ----------------------------------------------------------
